...
export interface ElectronAPI {
  ...
  agentListLogs: (
    maxCount?: number
  ) => Promise<Array<{ path: string; name: string; size: number; modified: Date }>>;
  ...
  voiceInterviewStreamingCreate: (input: {
    sessionId: string;
  ...
  onVoiceInterviewStateChange: (
    callback: (sessionId: string, state: unknown) => void
  ) => () => void;
  ...
  pdfGetTemplates: () => Promise<ResumeTemplate[]>;
  pdfGenerate: (
    resume: Resume,
    templateId: string,
