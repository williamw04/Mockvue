  ...
  if (!analysis) {
    return (
      <aside className="border-l border-rule bg-card overflow-y-auto px-5 py-6">
        <div className="text-center text-ink-3 text-sm">Run analysis to see insights</div>
      </aside>
    );
  }
        ...
        <div>
          ...
          <div className="space-y-2">
            {keywords.map((kw, i) => (
              <div
                key={i}
                className="flex items-center gap-2 py-1.5 border-b border-rule last:border-0"
              >
                <div
                ...
                {!kw.found && (
                  <span className="font-mono text-[9px] text-[#b91c1c] tracking-wide">MISSING</span>
                )}
              </div>
            ))}
          </div>
