...
export default function ResumeArchitectPage() {
  ...
  const handleRemoveCompanyChat = useCallback((chatId: string) => {
  ...
  const handleUploadJobDescription = useCallback((chatId: string, text: string) => {
    setCompanyChats(prev => prev.map(c =>
      c.id === chatId ? { ...c, jobDescription: text } : c
    ));
  }, []);
  ...
  const acceptedCount = allBullets.filter(b =>
    bulletStates[b.experienceId + '-' + b.bulletIndex]?.status !== 'pending'
  ).length;
    ...
    <div className="min-h-screen bg-bg flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-3 bg-card border-b border-rule flex items-center gap-4 flex-shrink-0">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[10px] text-ink-3 tracking-widest uppercase">RESUME ARCHITECT</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[11px] text-accent-hi font-semibold">{progressPercent}%</span>
        </div>

        <span className="font-mono text-[10px] text-ink-3 tracking-wider">
          {analysis ? `ANALYZED ${new Date(analysis.analyzedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}` : 'NOT ANALYZED'}
        </span>
      </div>
