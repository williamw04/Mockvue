...
export function SectionNav({ sections, activeSection, onSelectSection, bulletStates }: SectionNavProps) {
  const stats = {
    bullets: sections.reduce((sum, s) => sum + s.bullets.length, 0),
    accepted: sections.reduce((sum, s) => 
      sum + s.bullets.filter(b => bulletStates[b.experienceId + '-' + b.bulletIndex]?.status !== 'pending').length, 0),
  };
  ...
  return (
    <aside className="border-r border-rule bg-card overflow-y-auto py-5">
      ...
      {sections.map(section => {
        const doneCount = section.bullets.filter(b => 
