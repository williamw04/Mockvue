    ...
    >
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {todayEvents.map((event) => (
              ...
              >
                ...
                <div className={`text-sm truncate ${event.completed ? 'line-through text-muted-foreground' : ''}`}>
                  {event.title}
                </div>
              </div>
            ))}
      ...
      {isHovered && (
        <div className="absolute top-3 right-3 flex items-center gap-1 z-10">
          <button 
            className="w-8 h-8 rounded-md bg-background border shadow-sm hover:bg-accent flex items-center justify-center transition-colors"
