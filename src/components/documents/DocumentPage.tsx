    ...
    <div className="font-sans bg-bg text-ink w-full h-screen flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-3.5 border-b border-rule bg-card flex items-center gap-4 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div
            className="font-serif text-[16px] font-medium cursor-pointer"
            onClick={() => navigate('/document')}
          >
            Mockvue
          </div>
      ...
      <div className="flex-1 grid grid-cols-[240px_1fr] overflow-hidden">
        ...
        <aside className="border-r border-rule bg-card overflow-y-auto py-5">
          ...
          {MV_SHEET_PROGRESS.map((sec, i) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`flex-1 text-[13px] ${sec.locked ? 'text-ink-3' : 'text-ink'} ${isActive ? 'font-semibold' : 'font-normal'}`}
                >
                  {sec.name}
                </div>
                {sec.locked ? (
                  ...
                  <span
                    className={`font-mono text-[10px] ${sec.pct === 100 ? 'text-[#3d8a4a]' : 'text-accent-hi'}`}
                  >
                    {sec.pct}%
                  </span>
                )}
          ...
          <div className="p-5 mt-2.5 border-t border-rule">
            <div className="font-mono text-[10px] text-ink-3 tracking-[1.5px] mb-2.5">GAPS</div>
            <div className="p-2.5 bg-accent-lo border-l-2 border-accent-hi text-[12px] text-ink leading-relaxed">
              Missing a <b className="font-semibold">"Bias for Impact"</b> story. Draft before
              Wednesday.
            </div>
          </div>
        </aside>
          ...
          <div className="max-w-[720px] mx-auto px-[60px] py-[48px] pb-[80px]">
            ...
            <Dl
              rows={[
                ...
                [
                  'Values / Principles',
                  s.snapshot.values.map((v, i) => (
                    <em key={i} className="font-serif mr-2.5">
                      "{v}"
                    </em>
                  )),
                ],
            ...
            <ul className="m-0 mb-4.5 pl-4.5 text-[14px] leading-[1.7] text-ink-2 list-disc">
              {s.role.responsibilities.map((r, i) => (
                <li key={i} className="mb-0.5">
                  {r}
                </li>
              ))}
            ...
            <div className="flex flex-wrap gap-1.5 mb-4.5">
              {s.role.skills.map((k, i) => (
                <span
                  key={i}
                  className="px-2.5 py-1 bg-card border border-rule text-[12px] text-ink"
                >
                  {k}
                </span>
              ))}
            ...
            <div className="grid grid-cols-[80px_1fr] gap-y-1.5 gap-x-3.5 mb-4.5 text-[14px] leading-relaxed text-ink-2">
              <div className="font-mono text-[11px] text-ink-3">0–3 MO</div>
              <div>{s.role.success.m3}</div>
              <div className="font-mono text-[11px] text-ink-3">3–6 MO</div>
              <div>{s.role.success.m6}</div>
              <div className="font-mono text-[11px] text-ink-3">6–12 MO</div>
              <div>{s.role.success.m12}</div>
            </div>
            ...
            <div className="border border-rule bg-card mb-4.5">
              {s.role.signals.map((sig, i) => (
                <div
                  key={i}
                  className={`p-2.5 px-3.5 ${i > 0 ? 'border-t border-rule' : ''} grid grid-cols-[1fr_1fr_60px] gap-3 items-center text-[13px]`}
                >
                  <div className="font-medium text-ink">{sig.s}</div>
                  <div className="text-ink-3 italic">{sig.evidence}</div>
                  <div
                    className={`font-mono text-[10px] text-right ${sig.p === 'High' ? 'text-accent-hi' : 'text-ink-2'}`}
                  >
                    {sig.p.toUpperCase()}
                  </div>
                </div>
              ...
              {s.questionMap.map((q, i) => (
                <div key={i} className={`p-3.5 px-4 ${i > 0 ? 'border-t border-rule' : ''}`}>
                  ...
                  <div className="flex gap-5 text-[12px]">
                    <div>
                      <span className="font-mono text-[10px] text-accent-hi tracking-[1px] mr-1.5">
                        PRIMARY
                      </span>
                      <b className="font-semibold text-ink">{q.primary}</b>
                    </div>
                    <div className={q.backup ? 'text-ink-2' : 'text-ink-3'}>
                      <span className="font-mono text-[10px] text-ink-3 tracking-[1px] mr-1.5">
                        BACKUP
                      </span>
            ...
            <div className="border border-rule bg-card mb-4.5">
              {s.alignment.valueToStory.map((a, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`p-3 px-3.5 ${i > 0 ? 'border-t border-rule' : ''} grid grid-cols-[200px_1fr_80px] gap-3.5 items-center`}
                  >
                    <div className="font-serif text-[14px] italic text-ink">"{a.v}"</div>
                    <div
                      className={`text-[13px] ${a.story ? 'text-ink not-italic' : 'text-accent-hi italic'}`}
                    >
            ...
            <ul className="m-0 mb-4.5 pl-4.5 text-[14px] leading-[1.7] text-ink-2 list-disc">
              {s.alignment.mostRelevant.map((x, i) => (
                <li key={i} className="mb-0.5">
                  {x}
                </li>
              ))}
            ...
            <div className="p-6 border-[1.5px] border-dashed border-ink-3 bg-[rgba(0,0,0,0.02)] text-center">
              <div className="text-[13px] text-ink-3 font-serif italic">
                Unlocks after Thursday, Apr 24 at 2:45 PM.
                <br />
                You'll be prompted to fill: what went well, what didn't, questions you struggled
                with, stories to add.
              </div>
            </div>
  ...
  return (
    <div className="my-[34px] mb-[18px]">
      <div className="flex items-baseline gap-3">
        ...
        <h2
          className={`font-serif text-[28px] font-medium m-0 tracking-[-0.4px] ${locked ? 'text-ink-3' : 'text-ink'}`}
        >
          {title}
        </h2>
    ...
    <div className="grid grid-cols-[160px_1fr] gap-y-1.5 gap-x-4 mb-4.5">
      {rows.map(([k, v], i) => (
        <Fragment key={i}>
          ...
          <div className="text-[14px] text-ink-2 leading-relaxed">{v}</div>
        </Fragment>
      ))}
    </div>
  );
      ...
      <div className="flex items-baseline justify-between mb-2">
        ...
        <div className="flex gap-1">
          {story.tags.map((tag: string, i: number) => (
            <span
              key={i}
              className="text-[10px] font-mono py-[2px] px-1.5 bg-bg text-ink-2 tracking-[0.5px]"
            >
              {tag}
            </span>
          ))}
...
function TalkingCard({ label, text }: any) {
  return (
    <div className="bg-card border border-rule p-3.5 px-4 mb-3">
      ...
      <div className="font-serif text-[15px] leading-[1.55] text-ink">{text}</div>
    </div>
  );
}
