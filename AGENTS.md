...
- **Electron 28** for desktop, **browser** for web
...
The **document editor** (`src/components/documents/`) defines the canonical style and text patterns. All new UI must match its look and feel. See `docs/FRONTEND.md` for the full design system.

Key rules:
- **Light-mode only** — no dark mode, no ThemeContext usage
- Page bg: `bg-gray-100`, Cards/panels: `bg-surface` (#fafbfc, softer than pure white)
- Primary actions: `bg-blue-600 hover:bg-blue-700 text-white rounded-lg`
- Secondary actions: `bg-surface border-gray-200 hover:bg-gray-50 text-gray-700`
- Icons: Lucide React (`w-4 h-4` small, `w-5 h-5` default)
- System font stack, titles `text-3xl font-bold`, body `text-sm`

### When Implementation Fails
If you're stuck:
1. Check if required service methods exist in `src/services/interfaces.ts`
2. Verify platform-specific implementations exist in `src/services/electron/` and `src/services/web/`
